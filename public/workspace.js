...
function statusLabel(status) { return status === 'standby' ? 'En espera · plazo pausado' : status === 'completed' ? 'Completado' : status === 'delayed' ? 'Retrasado' : 'Activo'; }
function isCompleted(project) { return project.status === 'completed' || progress(project).percent === 100; }
...
function updateMetrics() {
  const delayed = state.projects.filter((project) => project.status === 'delayed' || (project.due_date && project.status === 'active' && new Date(`${project.due_date}T23:59:59`) < new Date())).length;
  $('#metric-active').textContent = state.projects.filter((project) => project.status === 'active' && !isCompleted(project)).length;
  $('#metric-completed').textContent = state.projects.filter(isCompleted).length;
  $('#metric-standby').textContent = state.projects.filter((project) => project.status === 'standby').length;
  $('#metric-delayed').textContent = delayed;
}
...
function visibleProjects() {
  ...
  const projects = state.projects.filter((project) => (status === 'all' || project.status === status) && `${project.title} ${project.description || ''}`.toLowerCase().includes(search));
  return projects.sort((first, second) => order === 'progress' ? progress(second).percent - progress(first).percent : order === 'due' ? String(first.due_date || '9999').localeCompare(String(second.due_date || '9999')) : new Date(second.created_at) - new Date(first.created_at));
}
...
function emptyState() { return `<section class="empty-projects"><div class="empty-folder">▱</div><h2>Aún no tienes proyectos</h2><p>Crea tu primer proyecto para comenzar a gestionar tus tareas y avances.</p><button class="button" id="empty-new-project">＋ Crear mi primer proyecto</button></section>`; }
function render() {
  const container = $('#projects');
  const projects = visibleProjects();
  container.innerHTML = projects.length ? '' : emptyState();
  if (!projects.length) { $('#empty-new-project').addEventListener('click', () => $('#project-dialog').showModal()); return; }
  projects.forEach((project) => {
    ...
    card.querySelector('.status').textContent = statusLabel(project.status);
...
setInterval(() => { $('#live-clock').textContent = new Intl.DateTimeFormat('es-PE', { dateStyle: 'full', timeStyle: 'short' }).format(new Date()); }, 1000);
